...
/*   Updated: 2025/11/22 21:55:31 by dnahon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	error_texture_no(t_map *map, t_data *data)
{
	...
	{
		free_parsing(data);
		printf(BOLD RED ERR_NORTH_TEXTURE RESET);
		exit_safe(map, 1);
	}
...
void	error_texture_ea(t_map *map, t_data *data)
{
	...
	{
		free_parsing(data);
		printf(BOLD RED ERR_EAST_TEXTURE RESET);
		exit_safe(map, 1);
	}
...
void	error_texture_we(t_map *map, t_data *data)
{
	...
	{
		free_parsing(data);
		printf(BOLD RED ERR_WEST_TEXTURE RESET);
		exit_safe(map, 1);
	}
...
void	error_texture_so(t_map *map, t_data *data)
{
	...
	{
		free_parsing(data);
		printf(BOLD RED ERR_SOUTH_TEXTURE RESET);
		exit_safe(map, 1);
	}
